...
import javafx.scene.control.{Button, ChoiceBox, DatePicker, ListView, TextField}
...
class RemindersController extends Initializable {
  ...
  @FXML private var text_field: TextField = _
  ...
  def add_func(): Unit = {
    ...
    val body = text_field.getText.trim
    remindersListView.getItems add ((title, body, priority, date_box.getValue, 0))
  }
